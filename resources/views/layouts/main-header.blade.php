									...
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
											<h6 class="dropdown-title mb-1 tx-15 text-white font-weight-semibold">الأشعارات</h6>
											    <span class="badge badge-pill badge-warning mr-auto my-auto float-left" id="span1"><a type="button" id="markAll">تحديد الكل مقروء</a></span>
										</div>
											@if(auth()->user()->unreadNotifications->count() == 0)
                                            	<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 text-center" id="span2">لا يوجد إشعارات</p>
											@endif
									</div>

                                        <div class="main-notification-list Notification-scroll" id="ss">
...
if(i === 1){
	...
	$.ajax({
		url: "{{ route('getUnreadNotificationsall') }}",
		type: "GET",
		dataType: "json",
		success: function(response) {
			...
			if(notifications == 0){
				$("#span1").addClass("hider");
			}else{
				$("#span1").removeClass("hider");
			}

			//console.log('majd',notifications);

			$("#countN").text(response.count);
			for(let notifi of notifications){

				let data = JSON.parse(notifi.data);

				let ress = `
				<a class="d-flex p-3 border-bottom" href="/invoice_details/${data.id}">
					<div class="notifyimg bg-pink">
						<i class="la la-file-alt text-white"></i>
					</div>
					<div class="mr-3">
						<h5 class="notification-label mb-1">${data.title}${data.user}</h5>
...
var invoice_id = payload.data.invoice_id;
const noteOptions = {
...
var ress = `
	<a class="d-flex p-3 border-bottom" href="/invoice_details/${invoice_id}">
		<div class="notifyimg bg-pink">
			<i class="la la-file-alt text-white"></i>
		</div>
		<div class="mr-3">
			<h5 class="notification-label mb-1">${noteTitle}</h5>
			<div class="notification-subtext">${noteBody}</div>
		</div>
	...
	$.ajax({
			url: "{{ route('getUnreadNotifications') }}",
			type: "GET",
			dataType: "json",
			success: function(response) {
				$("#countN").text(response.data);
				$('#ss').prepend(ress);
				console.log(ress);
			},
